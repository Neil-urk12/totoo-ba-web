...
const ProductVerify = async (product_id: string) => {
  const base = ((import.meta.env.VITE_GET_PRODUCT_VERIFY as string) || '')
    .replace('0.0.0.0', '127.0.0.1')
    .replace(/\/$/, '');

  const res = await fetch(`${base}/${encodeURIComponent(product_id)}`, { method: "GET" });
...
export const useGetProductVerifyQuery = (product_id: string) =>
  queryOptions({ queryKey: ["verify", product_id], queryFn: () => ProductVerify(product_id) });
